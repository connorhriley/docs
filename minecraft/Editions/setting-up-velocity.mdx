...
Velocity is a high-performance and modern proxy created by the team behind PaperMC. It's a growing solution that replaces Waterfall and even the industry-standard Bungeecord.
...
Once you have created your server you can then install Velocity by deleting all the files, and uploading the `.jar` file to the root directory. Then rename it to `velocity.jar` for simplicity and head to the **Startup** tab. 

Find the `Server Jar File` tab and make sure the field says `velocity.jar`. 

You can now start your Velocity server and once it's running head back to your File manager and your files should look like the following:

**IMAGE**

You can find all the configurations for Velocity in the `velocity.toml` file, this is what the next step will cover. 
